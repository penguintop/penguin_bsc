...
func (c *command) setAllFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Uint64(optionNameNetworkID, 530, "ID of the Penguin network")
	...
	cmd.Flags().String(optionNamePaymentThreshold, "10000000000000", "threshold in PEN where you expect to get paid from your peers")
	cmd.Flags().String(optionNamePaymentTolerance, "10000000000000", "excess debt above payment threshold in PEN where you disconnect from your peer")
	cmd.Flags().String(optionNamePaymentEarly, "1000000000000", "amount in PEN below the peers payment threshold when we initiate settlement")
