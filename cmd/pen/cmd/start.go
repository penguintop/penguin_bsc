...
import (
	...
	"github.com/penguintop/penguin_bsc/pkg/node"
...
HHHHHHHHHHHHHHHHHHHHH
		   ...
		   `
			...
			logger.Infof("version: %v", pen_bsc.Version)
			...
			b, err := node.NewPen(c.config.GetString(optionNameP2PAddr), signerConfig.address, *signerConfig.publicKey, signerConfig.signer, c.config.GetUint64(optionNameNetworkID), logger, signerConfig.libp2pPrivateKey, signerConfig.pssPrivateKey, node.Options{
...
func (c *command) configureSigner(cmd *cobra.Command, logger logging.Logger) (config *signerConfig, err error) {
	...
	if c.config.GetBool(optionNameClefSignerEnable) {
		...
		if created {
			logger.Infof("new penguin network address created: %s", address.String())
			logger.Infof("using ethereum address %s", overlayEthAddress.String())
			...
			logger.Infof("using existing penguin network address: %s", address.String())
			logger.Infof("using ethereum address %s", overlayEthAddress.String())
		}
	}
