...
func InitChain(
	...
	blocktime uint64,
) (*ethclient.Client, common.Address, penguin.Address, int64, transaction.Monitor, transaction.Service, error) {
	...
	if err != nil {
		return nil, common.Address{}, penguin.Address{}, 0, nil, nil, fmt.Errorf("eth address: %w", err)
	}

	penguinNodeAddress := crypto.NewOverlayFromEthereumAddress(overlayEthAddress[:], chainID.Uint64())
	...
	if !isSynced {
		logger.Infof("waiting to sync with the BSC backend")
	...
	return backend, overlayEthAddress, penguinNodeAddress, chainID.Int64(), transactionMonitor, transactionService, nil
}
