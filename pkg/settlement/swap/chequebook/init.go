...
import (
	...
	"github.com/penguintop/penguin_bsc/pkg/settlement/swap/erc20"
	...
	for {
		...
		if insufficientERC20 || insufficientETH {
			...
			if insufficientETH && insufficientERC20 {
				logger.Warningf("cannot continue until there is sufficient ETH (for Gas) and at least %d PEN available on %x", neededERC20, overlayEthAddress)
				...
				logger.Warningf("cannot continue until there is at least %d PEN available on %x", neededERC20, overlayEthAddress)
			}
			if chainId == 5 {
				logger.Warningf("learn how to fund your node by visiting our docs")
			}
			select {
